...
//	  struct Buffer buffer[numOfReducers];
	...
	//  char space = "";
      ...
      printf("Hello World! I am Mapper thread!\n");
      ...
      while (1) {
		  ...
		  if(read == -1){
			  ...
			  for(i=0; i<numOfReducers; i++){
				  ...
				  pthread_cond_signal(&buffer[i].condE);
			  }
//			  unlock(&mtx);
//			  lock = 0;
			  break;
	...
	//        printf("%s ", each);
	        ...
	        char q = *each;
            printf("qqqqqqqqqqqqqqqqqqqqqqqqq = %c ", q);
...
//	        rdrNum = findRdrNum(userId);
	        ...
	        free(str3);
	        free(str4);
	        pthread_cond_signal(&buffer[rdrNum].condE);
...
//       while(buffer[tid].Tuples[outSlotIndex[tid]] && done == 0)
       while(buffer[tid].count == 0 && done == 0){
    	   printf("done = %d\n", done);
    	   pthread_cond_wait(&buffer[tid].condE, &buffer[tid].mtx);
       }
...
//       buffer[tid].Tuples[outSlotIndex[tid]] = NULL;
